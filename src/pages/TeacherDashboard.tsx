          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-bold text-[var(--text-main)]">{auth.currentUser?.displayName || 'Teacher'}</p>
                <p className="text-[10px] text-[var(--text-sub)]">{auth.currentUser?.email}</p>
              </div>
              <div className="w-10 h-10 rounded-full bg-[var(--bg)] border border-[var(--border)] flex items-center justify-center font-black text-[var(--primary)] text-sm shadow-sm transition-transform hover:scale-105">
                {auth.currentUser?.displayName?.charAt(0) || auth.currentUser?.email?.charAt(0) || 'T'}
              </div>
            </div>
          </div>
        </header>
